 // Data set (sample) — used as fallback if no backend data
 const STATIC_RECIPES = [
  ...
  let currentPage = 1;
  let currentQuery = "";
  let FAVOURITES = []; // when loaded from backend
  ...
  function filterData(query) {
    ...
    return list.filter(r => {
      const title = (r.title || r.recipe_title || '').toLowerCase();
      const desc = (r.desc || r.recipe_text || '').toLowerCase();
      const tags = Array.isArray(r.tags) ? r.tags : [];
      return title.includes(q) || desc.includes(q) || tags.some(t => String(t).toLowerCase().includes(q));
    });
  }
  ...
  function renderCards(data, page) {
    ...
    for (const r of pageItems) {
      const id = r.id ?? r.favourite_id ?? Math.random();
      const title = r.title || r.recipe_title || 'Recipe';
      const desc = r.desc || r.recipe_text || '';
      ...
      card.innerHTML = `
        <article class="recipe-card h-100 position-relative">
          <button class="bookmark-btn ${isBookmarked ? "active":""}" aria-label="${isBookmarked ? "Remove bookmark":"Save bookmark"}" data-id="${id}">
  ...
  // Load favourites from backend if logged in
  (function loadBackendFavourites() {
    ...
    fetch(`${API_BASE}/favourites/${user.id}`)
      .then(async (res) => {
        ...
        // Normalize to structure expected by renderer
        FAVOURITES = Array.isArray(data) ? data.map((row, idx) => ({
          id: row.id || row.favourite_id || idx + 1,
          title: row.recipe_title,
          desc: row.recipe_text,
          time: null,
          servings: null,
          tags: []
        })) : [];
        update(1, currentQuery);
      })
      .catch(() => {
